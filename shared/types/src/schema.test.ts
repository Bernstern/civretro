...
describe('known nullability', () => {
  ...
  it('accepts legacyScore === null as well as 0', () => {
    expect(PlayerSnapshotSchema.safeParse({ ...recorderPlayer, legacyScore: null }).success).toBe(
      true,
    );
    expect(PlayerSnapshotSchema.safeParse({ ...recorderPlayer, legacyScore: 0 }).success).toBe(
      true,
    );
  });
  ...
  it('accepts the -9999 garrison/at-sea unit sentinel', () => {
    const player = {
      ...recorderPlayer,
      units: [{ typeName: 'UNIT_X', x: -9999, y: -9999, dmg: null, owner: 0 }],
    };
