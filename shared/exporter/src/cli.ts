import { type Db, getAllTurns, getIndex, getSession, getTurn, openDb } from './db.js';
import { ageName, buildRecords, toNdjson, turnSummary } from './export.js';

// DB path on Windows, reached through WSL. Mirrors the original read_localstorage.py.
...
const printAll = (db: Db): void => {
  ...
  if (index) {
    console.log(
      `[index] sessionId=${index.sessionId} totalTurns=${index.totalTurns} latest=${index.latest}`,
    );
  }
...
const main = (): void => {
  ...
  try {
    ...
    console.error(`ERROR: cannot open LocalStorage.sqlite at ${args.db}: ${errMessage(e)}`);
    process.exit(1);
  }
