...
// table (columns: id, key, value). Mirrors the original read_localstorage.py.
...
// better-sqlite3 returns rows as `unknown`; parse them at this boundary.
const ValueRowSchema = z.object({ value: z.string() });
const KeyValueRowSchema = z.object({ key: z.string(), value: z.string() });

const readValue = (db: Db, key: string): unknown => {
  const row = ValueRowSchema.safeParse(
    db.prepare('SELECT value FROM "Values" WHERE id = ? AND key = ?').get(ORIGIN, key),
  );
  return row.success ? JSON.parse(row.data.value) : null;
};

const readPrefix = (db: Db, prefix: string): readonly z.infer<typeof KeyValueRowSchema>[] =>
  z
    .array(KeyValueRowSchema)
    .parse(
      db
...
export const getTurn = (db: Db, globalTurn: number): TurnSnapshot | null => {
  ...
  if (!parsed.success)
    throw new Error(`Failed to read turn ${globalTurn}: ${parsed.error.message}`);
  return parsed.data;
};
...
export const getAllMapSnapshots = (db: Db): MapSnapshot[] =>
  readPrefix(db, MAP_PREFIX)
    .map(({ key, value }) => {
      ...
      if (!parsed.success)
        throw new Error(`Failed to read map snapshot ${key}: ${parsed.error.message}`);
      return parsed.data;
    })
